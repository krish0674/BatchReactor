...
class BR3(gym.Env):
    ...
    def step(self, action):
        ...
        # if error <= 2: #and we need to change how much reward we assign
        #     self.reward = 100
        # elif error <= 1:
        #     self.reward = 50
        # else:
        #     self.reward = -error
        self.reward=-error*100+10
...
def get_actor():
    model = Koopman()
    model.load_state_dict(torch.load(r'C:\Users\KRISH DIDWANIA\BatchReactor-5\DDPG.py\best_model (3).pth'))
    return model
...
def scale_state(state):
    state = (state - torch.tensor([34.946917, 30.485979])) / (torch.tensor([90.839534, 60.022752]) - torch.tensor([34.946917, 30.485979]))
    return 2.0 * state - 1.0

# Policy function
def policy(state,paction, noise_object, actor_model):
    state = torch.tensor(state, dtype=torch.float32)
    action = actor_model(state,paction)
...
def ddpg(env):
    ...
    for ep in range(total_episodes):
        ...
        while True:
            ...
            action = policy(torch_prev_state, prev_action, ou_noise, actor_model)
            ...
            if buffer.buffer_counter > buffer.batch_size:
                ...
                # Critic update
                with torch.no_grad():
                    target_actions = target_actor(next_state_batch,action_batch)
                ...
                actions = actor_model(state_batch,action_batch)
